...
router.post('/:id/add-payment', async (req, res) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const totalPayments = updatedRental!.payments.reduce((s, p) => s + p.amount, 0);
      const manual = (updatedRental!.upfront ?? 0) + (updatedRental!.pay1 ?? 0) + (updatedRental!.pay2 ?? 0) + (updatedRental!.pay3 ?? 0) + (updatedRental!.pay4 ?? 0);
      const balance = Math.max(0, updatedRental!.totalDue - (totalPayments + manual));
